...
class Deploy:
    ...
    def upload_package(self):
        local_file_path = LOCAL_PATH_FORMAT.format(function_name=self.function_name)
        ...
        print(
            "Uploading {} to {}".format(
                local_file_path, "{}:::{}".format(S3_BUCKET, key)
            )
        )
    ...
    def schedule_expression_to_event_rule_name(self, expression):
        split_expression = (
            expression.replace("*", "x").replace("?", "q").strip(")").split("(")
        )
        expression_type = split_expression[0]
        expression_statement = "_".join(
            split_expression[1].replace(",", " ").split(" ")
        )
        return "{}-{}".format(expression_type, expression_statement)
    ...
    def remove_target_from_event_triggers(self, function_arn, event_names):
        for event_name in event_names:
            ...
            self.events_client.remove_targets(Rule=event_name, Ids=[target_id])

    def create_event_trigger(self, event_rule_name, schedule_expression):
        response = self.events_client.put_rule(
            Name=event_rule_name, ScheduleExpression=schedule_expression
        )
    ...
    def add_trigger_to_event(self, event_rule_name, rule_arn, function_arn):
        self.events_client.put_targets(
            ...
            Targets=[{"Id": self.function_name, "Arn": function_arn}],
        )
    ...
    def _run(self):
        ...
        for schedule_expression in self.config["schedule_expressions"]:
            ...
            event_rule_name = self.schedule_expression_to_event_rule_name(
                schedule_expression
            )
            event_rule_arn = self.event_rule_arn(event_rule_name)
            if not event_rule_arn:
                print("   Event does not exist - creating")
                event_rule_arn = self.create_event_trigger(
                    event_rule_name, schedule_expression
                )
    ...
    def run(self):
        ...
        print("Invoke with:")
        print(
            "   python invoke.py {function_name} {profile}".format(
                function_name=self.function_name,
                profile=(
                    "--profile={}".format(self.aws_profile) if self.aws_profile else ""
                ),
            )
        )
        print("")
